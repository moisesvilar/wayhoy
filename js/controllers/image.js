angular.module('angularApp').controller('ImageController', [
    ...
    'TOKEN',
	'ACCESS_POINT',
    function($scope, $location, $routeParams, $http, store, md5, TokenService, ImageService, AlbumService, AlbumImageService, PermissionsService, USER, PASSWORD, TOKEN, ACCESS_POINT) {
        ...
        TokenService.get(function(json) {
			...
			if (imageId){
                $scope.model.albums = AlbumService.query(function() {
                    console.log($scope.model.albums);
                });
        ...
        $scope.myAlbums = function() {
            if($scope.model.albums && $scope.model.albums.length > 0) {
                return $scope.model.albums.filter(function(item) {
                    return item.is_mine === '1';
                });
            }
            return [];
        };
		...
		$scope.duplica = function(i, t, d, s) {
			...
			$scope.addimage(i, albumIdLuk, null, t, d, s, 'du');
		//console.log(i, albumIdLuk, null, t, d, s);
        }
		$scope.quealbum = function(a) {
			albumIdLuk=a;
        }
$scope.addimage = function(i,a, r, t, d, s, du) {
	console.log(i, a, r, t, d, s);
    ...
    var addUrl = ACCESS_POINT+'/mobile/luk/imageaddalbum.php?u=' + encodeURIComponent(user) + '&t=' + pass + '&image=' + i + '&album=' + a + joiner + '&title=' + ti + '&desc=' + de + '&subdesc=' + su;
	console.log(addUrl);
	$.ajax({
		url : addUrl,
		type : 'get',
